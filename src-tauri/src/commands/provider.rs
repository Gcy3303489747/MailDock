...
pub fn sync_qq_inbox(app: AppHandle, input: QqInboxSyncInput) -> Result<QqInboxSyncReport, String> {
    ...
    save_qq_authorization_code(&credentials, account_id, &address, &authorization_code)?;
...
pub fn sync_saved_qq_inbox(
    ...
    let authorization_code =
        saved_qq_authorization_code(&credentials, &credential_key, account.id)?;
...
fn save_qq_authorization_code(
    ...
    let saved = saved_qq_authorization_code(credentials, &primary_key, account_id)?;
